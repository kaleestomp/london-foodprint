...
legacy/etl_load.py
------------------
Legacy ETL script that builds and loads both `places` and `h3_density` into PostgreSQL/Neon.
Archived for reference in case the pre-aggregated `h3_density` pipeline is needed in the future.
-----------
...
SERVER_ROOT = Path(__file__).resolve().parents[2]  # legacy/ -> db/ -> server/
...
from db.legacy.etl.insert_places import insert_places
from db.legacy.etl.insert_h3_density import insert_h3_density
