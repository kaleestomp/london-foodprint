-- London Explorer — Legacy Cloud DB Schema (with h3_density)
-- Archived for reference in case pre-aggregated H3 density table is needed in the future.
-- Run once against your Neon database.
-- PostGIS is natively supported on Neon; H3 math runs in Python (no h3-pg needed).
--
-- Design notes:
--   • h3_r10  = H3 res-10 cell — used for nearby k-ring lookups.
--   • h3_density = Pre-aggregated tile counts across all combinations of filter dimensions
--                  (resolution, cuisine_type, cost, venue_type, score_basis, score_tier).
