...
const makePlacePinIcon = (startOffset?: { dx: number; dy: number }): L.DivIcon => {
  const animClass  = startOffset ? 'density-pin-fly-in' : 'density-pin-enter';
  const styleExtra = startOffset
    ? `--fly-dx:${startOffset.dx.toFixed(1)}px;--fly-dy:${startOffset.dy.toFixed(1)}px`
    : '';
  ...
  return L.divIcon({
    ...
    popupAnchor: [0, -PIN_H],
    html: `<div class="density-pin ${animClass}"${styleExtra ? ` style="${styleExtra}"` : ''}>
