...
const heatmapGeojson = (
    coordinates: { id: string; lat: number; lng: number; cuisine_type: string | null }[]
    ...
    const data = {
        ...
        features: coordinates.map(({ id, lat, lng, cuisine_type }) => ({
            type: 'Feature' as const,
            id,
            properties: { id, cuisine_type, weight: 1 + boost },
            geometry: {
                type: 'Point' as const,
                coordinates: [lng, lat],
            },
