# Restaurant Info Panel Rebuild Architecture
...
- `RestaurantInfoPanel` does not consume BubbleAvatar state.
- `BubbleAvatar` remains inside `RestaurantPanelSnapProvider` because BubbleAvatar internals read snap metrics (`isMobile`, `panelHeight`, `translateY`).
...
Notes:
- `RestaurantInfoPanel` depends on `RestaurantPanelSnapProvider`.
- `BubbleAvatar` depends on both `RestaurantPanelSnapProvider` and `BubbleAvatarStateProvider`.
- `RestaurantInfoPanel` does not require `BubbleAvatarStateProvider`.
