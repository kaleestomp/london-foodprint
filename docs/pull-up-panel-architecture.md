# PullUpPanel Architecture

## Scope
This document describes the current architecture of the PullUpPanel in the `MapPage` flow, including state ownership, hook boundaries, render structure, and request lifecycles.

## Main Files
- `src/MapPage/components/PullUpPanel/PullUpPanel.tsx`
- `src/MapPage/components/PullUpPanel/RestaurantList.tsx`
- `src/MapPage/components/PullUpPanel/BottomToolbar.tsx`
- `src/MapPage/components/PullUpPanel/SnapHooks/usePullUpPanelSnap.ts`
- `src/MapPage/components/PullUpPanel/SnapHooks/PullUpPanelSnapContext.tsx`
- `src/MapPage/components/PullUpPanel/RestaurantList/usePullUpPanelListQuery.ts`
- `src/MapPage/components/PullUpPanel/config.ts`

## Component Responsibility
`PullUpPanel.tsx` is primarily a layout and interaction wiring component.

It composes three concerns:
1. Panel pose and drag state from `usePullUpPanelSnapState()`
...
Owned by `usePullUpPanelSnap.ts`:
...
Owned by `usePullUpPanelListQuery.ts`:
...
`PullUpPanel` does not consume bubble state directly.

## Context Layer
`PullUpPanelSnapContext.tsx` wraps the snap hook and exposes a stable consumer API:
...
`BubbleAvatarStateProvider` is intentionally scoped to `BubbleAvatar` and not used by `PullUpPanel`.
...
## Data Request Flow
1. `usePullUpPanelListQuery` computes effective bounds from filters + map state
...
- `PullUpPanel`: mobile/desktop shell and drag-wiring
